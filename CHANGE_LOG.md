...
- [Bug 6525143] Added a hermetic unit contract test (`defaultGeneralPreChatSurveyPaneStyleProps.test.ts`) asserting the pre-chat pane fills via flex (`flexGrow`/`flexBasis`/`minHeight`) with its own scroll and no fixed/percentage height that could reintroduce the header clip
- [Bug 6525143] Added a real-browser layout regression e2e (`automation_tests/e2e/areas/general/preChatHeaderClip.spec.ts` + `PreChatSurveyHeaderClipWidget.html` fixture, gated on a built `dist/out.js`) that renders a pre-chat survey taller than the 560px container and asserts the header's top edge is not clipped, the header keeps a real height, and the overflow is absorbed by the pane's own scroll
- [Bug 6525143] Added a Storybook pixel-diff (VRT) scenario: story `LiveChatWidgetPreChatTallSurvey` (pre-chat survey taller than the 560px container) plus a `prechat tall survey header not clipped` screenshot config in `LiveChatWidget.stories.playwright.json`, so `yarn test:visual` captures/diffs the pre-chat surface. NOTE: the baseline PNG must be generated once locally (`yarn build-storybook && yarn test:visual`) and committed before it gates in CI

### Fixed
- [Bug] Pre-chat survey pane renders full-size without clipping the chat header. The focus-capture wrapper added around `<PreChatSurveyPane>` (to host the polite live region) previously declared `height: 100%`, which resolves to the full height of the centered flex-column widget container rather than the space below `HeaderStateful` — pushing header + pane past the container so the centered, clipped column cut off the header's top edge (Bug 6525143, regression from PR #957). Fixed by (1) sizing the wrapper to fill only the remaining space as a flex column (`flex: 1 1 auto; minHeight: 0; display: flex; flexDirection: column`), (2) making the inner pane fill that space via flex (`flexGrow: 1; flexBasis: 0; minHeight: 0` with its own `overflowY: auto`) instead of a percentage height that silently collapses when an ancestor height is indefinite (docked/mobile/first-paint), and (3) seeding `flexShrink: 0` on the header (overridable) so a pre-chat survey taller than the container scrolls inside the pane instead of stealing pixels from the header. Keeps the header fully visible while still preventing the collapse-to-content-size behavior (Bug 6423684)
